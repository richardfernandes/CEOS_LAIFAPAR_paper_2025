# CEOS_LAIFAPAR_paper_2025
Documents, data and results for CEOS LAI FAPAR validation paper 2025

## Welcome coauthors!  

This repo contains material for the CEOS WGCV Biophysical Group paper "A Methodology for Validating Synoptic Decametric Resolution Satellite Leaf Area Index and fAPAR Products."



The goal is to submit an invited paper **BEFORE February 28, 2026** to the [RSE Specical Issue Development, Validation, and Application of Medium- and High-Resolution Satellite Products at Regional to Global Scales](https://www.sciencedirect.com/special-issue/321741/development-validation-and-application-of-medium-and-high-resolution-satellite-products-at-regional-to-global-scales). 



We are going for a paper both for visibility and timeliness.  \
This paper fills a gap in current LPV methods.  \
Once published, the paper and code base would enable us to quickly update the current good practice guide and enhance the ESA Cal/Val portal code.






## **Due December 15, 2025:**

* Please review the [manuscript](https://github.com/richardfernandes/CEOS_LAIFAPAR_paper_2025/blob/main/manuscript/A%20Methodology%20for%20Validating%20Decametric%20Resolution%20Satellite%20Leaf%20Area%20Index%20and%20fAPAR%20Products_V01.docx)  and save a copy in the same folder without your initials and date as a suffix:
* * indicate general comments and suggest changes to sections, conceptual or theoretical issues, dissenting opinions including if need be new methods.
  * note the scope of the paper - you are most welcome to contribute your own papers if you feel the scope is too restrctive.
  * try NOT to suggest additional materials - the paper is about methods and not an actual validation report.
* Please log an **issue** in this repo if you have a strong opinion or feel there is a blunder of some sort




## Schedule:

* November 15, 2025 : Version 0 provided to coauthors.  
* **December 15, 2025 : Coauthors provide comment on Version 0, and if they wish  new materials and method via github.**
* January 15, 2025   : Version 1 provided to coauthors, includes results and some discussion.  
* **February 15, 2025 : Coauthors provide comment on Version 1 focussing on results, discussion and conclusion.**
* February 28, 2025 : Version 2 sumbitted to RSE.

I expect the schedule will slip about 1 month due to holidays but we shall see...






## **Folders (not all are populated as of November 15, 2025):**

* manuscript - materials for submission and review
* documents - supporting documents, mainly major references
* materials - materials identified in manuscript
* methods - methods identified in manuscript
* CEOS - ceos specific items

